...
class Wiki {
        ...
        switch ($action) {
            ...
            case 'history':
                ...
                foreach ( _wp_post_revision_fields() as $field => $field_title ) :
                    if ( 'diff' == $action ) {
                        $left_content = apply_filters( "_wp_post_revision_field_$field", $left_revision->$field, $field, $left_revision );
                        $right_content = apply_filters( "_wp_post_revision_field_$field", $right_revision->$field, $field, $right_revision );
                        if ( !$rcontent = wp_text_diff( $left_content, $right_content ) )
                            continue; // There is no difference between left and right
                        $identical = false;
                    } else {
                        add_filter( "_wp_post_revision_field_$field", 'htmlspecialchars' );
                        $rcontent = apply_filters( "_wp_post_revision_field_$field", $revision->$field, $field, $revision );
                    }
    ...
    function name_save($post_name) {
        if ( isset($_POST['post_type']) && $_POST['post_type'] == 'incsub_wiki' && empty($post_name) && isset($_POST['post_title']) ) {
